/* ==========================================================================
   MealUp App Engine - Complete Code
   ========================================================================== */
...
/**
 * Calcola l'indice Best Value (efficienza tempo vs nutrienti).
 * Formula: (Proteine / (Tempo Preparazione + Tempo Cottura + 1)) * (1000 / Calorie)
 */
...
/**
 * Filtra e applica il badge Best Value alle ricette top.
 */
function processBestValueBadges(recipesList) {
    if (!recipesList.length) return [];
    
    // Calcola punteggio per tutte
    ...
    // Assegna il badge se il punteggio è entro il top 15% del massimo o superiore a una soglia
    return scored.map(r => ({
        ...r,
...
/**
 * Renderizza l'elenco delle ricette applicando filtri, ordinamento e calcolo Best Value
 */
function renderRecipes() {
    const container = document.getElementById('recipes-container');
    if (!container) return;

    // 1. Applica filtro Blacklist rigoroso
    let filtered = filterRecipesByBlacklist(AppState.recipes, AppState.blacklist);

    // 2. Calcola e assegna i punteggi Best Value
    ...
    // 4. Filtro o Evidenziazione per Stato Giornata Allenamento
    if (AppState.workoutState !== 'rest') {
        // Ordina mettendo in cima quelle ideali per lo stato corrente
        filtered.sort((a, b) => {
    ...
    // 5. Ordinamento Utente (Default: Calorie Aumentate)
    filtered.sort((a, b) => {
...
/**
 * Dettaglio Ricetta Modal / Accordion
 */
...
function renderPantryGrid() {
    ...
    // Pulisci ingredienti selezionati se in blacklist
...
function renderPantryMatches() {
    ...
    // Calcola percentuale di affinità
...
/**
 * Apre le mappe del dispositivo (Apple Maps su iOS o Google Maps altrove)
 */
...
function handleAddRecipe(e) {
    ...
    // Controlla se contiene ingredienti vietati
